...
export const I18N = {
  ...
  settings: {
    ...
    promptSub: '默认读取 docs/anki_prompt.txt；可修改并保存自定义提示词，随 Gist 一并上传与拉取。',
    systemPrompt: 'System Prompt',
    systemPromptHint: '留空则使用默认提示词',
    promptPlaceholder: '在此输入自定义提示词，留空则回退到 docs/anki_prompt.txt。',
    promptStatusDefault: '当前使用：默认提示词（docs/anki_prompt.txt）',
  ...
  toast: {
    ...
    anki: {
      ...
      promptRestoredDefault: '已恢复默认提示词，后续处理将读取 docs/anki_prompt.txt。',
      configSaveFailed: 'Anki API 配置保存失败，请检查浏览器存储权限。',
      configSaved: 'Anki API 配置已保存。',
      defaultPromptEmpty: '默认提示词为空，请检查 docs/anki_prompt.txt。',
